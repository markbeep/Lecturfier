...
class Minesweeper(commands.Cog):
    ...
    async def minesweeper(self, ctx, board_size="10", mines="10"):
        """
        ...
        for row in mine_field:
            ...
            for i in row:
                if "f" in str(i):
                    ...
                    row_string += f"||{i}||"
            if len(minesweeper_text_format(final_message) + minesweeper_text_format(row_string)) > 750:
                msg = minesweeper_text_format(final_message)
                await ctx.send(msg)
        ...
        try:
            final_message = minesweeper_text_format(final_message)
            self.sending = False
...
def minesweeper_text_format(text):
    text = text.replace("x", "🟥")
...
def bomb_placer(size:int, mines:int):
    mine_field = []
    if size < 2:
        size = 2
    if mines < 1:
