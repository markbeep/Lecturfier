...
class Admin(commands.Cog):
    ...
    async def on_message_delete(self, message):
        ...
        if len(message.attachments) > 0:
            embed.add_field(name="Attachments", value=attach_txt)
    ...
    async def on_message(self, message):
        ...
        if message.channel.id in [
            ...
            if message.content.lower().startswith(
                "prefix"
            ) or message.content.lower().startswith("prefixes"):
    ...
    async def send_prefix(self, message, command=None, prefix=None, args=[]):
        ...
        if command is None:
            ...
            for prefix in self.all_prefix.keys():
                ...
                if len(res) > 0 and res[0].isnumeric():
                    ...
                    if user.status != discord.Status.offline:
                        online.append(
                            f"<:online:1015269202338263140> `{prefix}`: {prefix_line}"
                        )
                    else:
                        offline.append(
                            f"<:offline:1015269203684638720> `{prefix}`: {prefix_line}"
                        )
                else:
                    offline.append(
                        f"<:offline:1015269203684638720> `{prefix}`: {prefix_line}"
            ...
            if n > 10:  # make two fields next to each other
                ...
                embed.add_field(
                    name="\u200b",
                    value="\n".join(combined[:partition_size]),
                    inline=False,
                )
                embed.add_field(
                    name="\u200b",
                    value="\n".join(combined[partition_size : 2 * partition_size]),
                    inline=False,
                )
                embed.add_field(
                    name="\u200b",
                    value="\n".join(combined[2 * partition_size : 3 * partition_size]),
                    inline=False,
                )
                embed.add_field(
                    name="\u200b",
                    value="\n".join(combined[3 * partition_size :]),
                    inline=False,
                )
            else:
            ...
            or command.lower() == "del"
            and author.guild_permissions.kick_members
        ):
            if prefix is None:
    ...
    @commands.guild_only()
    @commands.command(
        aliases=["prefixes"], usage="prefix [<add/delete> <prefix> <info>]"
    )
    ...
    async def send_welcome_message(self, channel, user, guild):
        embed = discord.Embed(
            description=f"{user.mention} joined the server. **Welcome!**",
            color=0xADD8E6,
        )
        memb_amt = len(guild.members)
        embed.set_footer(text=f"There are now {memb_amt} members")
        embed.set_author(
            name=str(user), icon_url=user.avatar.url if user.avatar else None
        )
    ...
    async def send_leave_message(self, channel, user, guild):
        embed = discord.Embed(
            description=f"{user.mention} left the server.", color=0x84001B
        )
        memb_amt = len(guild.members)
        embed.set_footer(text=f"There are now {memb_amt} members")
        embed.set_author(
    ...
    async def ban(self, ctx, person, *, reason=None):
        ...
        embed = discord.Embed(
            description=f"Was just a prank brudi {person}", color=discord.Color.green()
        )
...
class WelcomeViewPersistent(discord.ui.View):
    def __init__(self):
        ...
        self.add_item(
            discord.ui.Button(
                label="Verify ETH Student",
                style=discord.ButtonStyle.url,
                emoji=YES_EMOJI_ID,
                url="https://dauth.spclr.ch/",
            )
        )

    @discord.ui.button(
        label="Don't verify",
        custom_id="welcome_view_persistent:dont_verify",
        style=discord.ButtonStyle.red,
        emoji=NO_EMOJI_ID,
    )
    async def dont_verify(self, interaction: discord.Interaction, _: discord.ui.Button):
        await interaction.response.send_message(
            view=WelcomeViewDecline(), ephemeral=True
        )

    @discord.ui.button(
        label="I need help",
        custom_id="welcome_view_persistent:help",
...
class WelcomeViewDecline(discord.ui.View):
    ...
    @discord.ui.button(
        ...
        custom_id="welcome_view_decline:skip_verify",
        style=discord.ButtonStyle.red,
        emoji=NO_EMOJI_ID,
    )
    async def skip_verify(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        try:
            ...
            await interaction.response.send_message(
                "External role not found. This is an internal mistake.", ephemeral=True
            )
            return
        # for testing purposes
        embed = discord.Embed(
            description=f"Added **External** role to {member.mention}\n"
            f"ID: `{member.id}`",
            color=0xA52222,
        )
        embed.set_author(
            name=str(member), icon_url=member.avatar.url if member.avatar else None
        )
...
class WelcomeViewHelp(discord.ui.View):
    ...
    @discord.ui.button(
        label="Verifying my ETH account",
        custom_id="welcome_view_help:verify",
...
**4.** You should now have access to tons of channels."""
    ...
    @discord.ui.button(
        label="Other",
        custom_id="welcome_view_help:other",
        style=discord.ButtonStyle.grey,
    )
    async def other(self, interaction: discord.Interaction, _: discord.ui.Button):
        ...
        if member.id in self.requested_help:
            await interaction.response.send_message(
                "You already requested help. Please wait.", ephemeral=True
            )
            return
        embed = discord.Embed(
        ...
        await staff_channel.send(
            f"||<@&844572520497020988>|| {member.mention}", embed=embed
        )
        await interaction.response.send_message(
            "The staff team was notified and will help you shortly. \
            You additionally should now have access to the <#881611441105416263> channel.",
            ephemeral=True,
        )
        ...
        await support_channel.set_permissions(
            member, read_messages=True, reason="User requested help"
        )
